...
import Router from '../core/route/mapper';
...
export default class RouteLoader{
  ...
  bindControllers(onRouteBinding) {
    for (const Controller of this.getControllers()) {
      console.info(`Routes for Controller [${Controller.name}] at ${Controller.prototype.route.path}`);

      const controller = new Controller();
      this.router.mapControllerActions(controller, function mapRouteConfig(actionConfig, controllerPropertyName) {
        console.info(` - Action handler [${actionConfig.method}][${controllerPropertyName}] at ${actionConfig.path}`);
