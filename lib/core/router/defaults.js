...
export default class DefaultRoutingOptions{
  ...
  _hasDefaultRoute(resourcePath, defaultRoute) {
    return /\/(index)\/?/i.replace(resourcePath, defaultRoute);
  }

  _hasDefaultRouteName(controllerName, defaultRoute) {
    return /(index)/i.test(controllerName, defaultRoute);
  }
