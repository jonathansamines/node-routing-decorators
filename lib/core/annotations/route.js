import {WebControllerAnnotation} from './controller';
import {HttpMethodAnnotation} from './httpMethod';
...
export default function RouteAnnotation(params) {
  ...
  return function buildRouteDecorator(...args) {
    ...
    if (args.length === 1) {
      ...
      return controller.resourceControllerBuilder.apply(controller, args);

    // Class method signature
    }else if (args.length === 3) {
      const method = new HttpMethodAnnotation(params);
      return method.resourceMethodBuilder.apply(method, args);
    }
